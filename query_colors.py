    ...
    if os.path.exists(COMBOS_FILE):
        ...
        for c in colors:
            for cid in c["combinations"]:
                ...
                combos[cid]["colors"].append({"index": c["index"], "name": c["name"], "hex": c["hex"]})
...
def format_palette(combo, colors_db):
    ...
    lines = [f"\n{'='*70}", f"  Combination #{cid}  |  {ctype}  |  {len(combo['colors'])} colors", f"{'='*70}"]
    for cc in combo["colors"]:
        full = next((c for c in colors_db if c["index"] == cc["index"]), None)
        lines.append(format_color(full) if full else f"  [{cc['index']:>3}] {cc['name']:<30} {cc['hex']}")
    ...
    if "contrast" in combo and combo["contrast"].get("pairs"):
        ...
        for p in ct["pairs"]:
            lines.append(f"    {icons.get(p['grade'],'')} {p['colors'][0]} ↔ {p['colors'][1]}: {p['ratio']}:1 ({p['grade']})")
        if ct.get("all_wcag_aa"): lines.append(f"  ✅ All pairs WCAG AA")
        elif ct.get("all_wcag_aa_large"): lines.append(f"  ⚠️  WCAG AA large text only")
    if combo.get("palette_temperature"): lines.append(f"  Temperature: {combo['palette_temperature']}")
    if combo.get("colorblind_safe") == False: lines.append(f"  ⚠️  Possible colorblind issues")
    if "curated" in combo:
        cur = combo["curated"]
        lines.append(f"\n  💡 {cur['context']}")
        lines.append(f"     Mood: {', '.join(cur['mood'])}  |  Best for: {', '.join(cur['domains'])}")
    return "\n".join(lines)
...
def cmd_palette(args, colors, combos):
    cid = int(args[0])
    print(format_palette(combos[cid], colors) if cid in combos else f"Combination #{cid} not found (1-348)")

def cmd_search(args, colors, combos):
    term = " ".join(args).lower()
    matches = [c for c in colors if term in c["name"].lower()]
    if not matches: print(f"No colors matching '{term}'"); return
    print(f"\n{len(matches)} color(s) matching '{term}':\n")
    for c in matches:
        print(format_color(c, lab=True))
        print(f"         {c.get('temperature','?')} | {c.get('lightness_class','?')} | {c.get('saturation_class','?')} | {c['use_count']} combos: {c['combinations']}")
...
def cmd_match(args, colors, combos):
    ...
    dists = sorted([(delta_e(target_lab, c.get("lab", list(rgb_to_lab(*c["rgb_array"])))), c) for c in colors], key=lambda x: x[0])
    ...
    for cid in best["combinations"][:5]:
        if cid in combos: print(format_palette(combos[cid], colors))
    if len(best["combinations"]) > 5: print(f"\n  ... and {len(best['combinations'])-5} more")

def cmd_random(args, colors, combos):
    ctype = args[0] if args else None
    pool = {k:v for k,v in combos.items() if not ctype or v.get("type")==ctype or (ctype=="duo" and 1<=k<=120) or (ctype=="trio" and 121<=k<=240) or (ctype=="quad" and 241<=k<=348)}
    cid = random.choice(list(pool.keys()))
    print(format_palette(pool[cid], colors))

MOOD_MAP = {
    "warm": ["red","orange","yellow","cinnamon","sienna","coral","rufous","golden","peach"],
    "cool": ["blue","green","turquoise","teal","cerulian","glaucous","nile","cobalt","olympic"],
    "earthy": ["brown","olive","sienna","umber","khaki","sepia","ochre","raw","sudan","citrine","maple","ecru"],
    "elegant": ["purple","violet","carmine","lake","vandyke","pansy","rose","lilac","mauve"],
    "soft": ["pink","lavender","pale","light","buff","cream","cameo","fawn","seashell"],
    "bold": ["spectrum","scarlet","red","orange","peach","jasper","grenadine","coral"],
    "natural": ["green","olive","citrine","rainette","pistachio","andover","artemesia","lincoln","oil"],
    "dramatic": ["black","deep","dark","vandyke","burnt","indigo","slate","pansy"],
    "fresh": ["green","turquoise","nile","calamine","venice","cobalt","mint","lemon","benzol"],
    "autumn": ["sienna","orange","rufous","brown","ocher","rust","ochre","brick","hay","burnt"],
    "spring": ["green","yellow","pink","lemon","turquoise","fresh","cobalt","pistachio"],
    "summer": ["blue","turquoise","sea","cerulian","olympic","calamine","nile","coral"],
    "winter": ["blue","violet","slate","gray","indigo","deep","dark","tyrian","plumbeous"],
    "minimal": ["white","black","gray","neutral","slate","mineral","warm gray"],
    "vintage": ["buff","ecru","ocher","sienna","fawn","isabella","drab","cameo","maple"],
    "tropical": ["coral","turquoise","orange","green","pink","lemon","sea","grenadine"],
    "luxurious": ["violet","purple","indigo","carmine","golden","deep","pansy","cotinga"],
}
...
def cmd_mood(args, colors, combos):
    ...
    if not terms:
        matching = set()
        for c in colors:
            if kw in c["name"].lower(): matching.update(c["combinations"])
        if not matching: print(f"No palettes for '{kw}'. Try: {', '.join(sorted(MOOD_MAP.keys()))}"); return
    else:
        matching = set()
        for c in colors:
            if any(t in c["name"].lower() for t in terms): matching.update(c["combinations"])
    ...
    sample = curated[:3] + random.sample(rest, min(max(0, 5-len(curated[:3])), len(rest)))
...
def cmd_popular(args, colors, combos):
    ...
    for c in sorted(colors, key=lambda c: c["use_count"], reverse=True)[:n]:
        print(f"  {format_color(c)}  ({c['use_count']} combos, {c.get('temperature','?')})")

def cmd_accessible(args, colors, combos):
    ctype = args[0] if args else None
    pool = [c for c in combos.values() if c.get("contrast",{}).get("all_wcag_aa") and c.get("color_count",0)>=2 and (not ctype or c.get("type")==ctype)]
...
def cmd_colorblind(args, colors, combos):
    ctype = args[0] if args else None
    pool = [c for c in combos.values() if c.get("colorblind_safe",True) and c.get("contrast",{}).get("all_wcag_aa_large") and (not ctype or c.get("type")==ctype)]
...
def cmd_curated(args, colors, combos):
    ...
    pool = [c for c in sorted(combos.values(), key=lambda x: x["id"]) if "curated" in c and (not domain or domain in c["curated"].get("domains",[]))]
    print(f"\n{len(pool)} curated palettes{f' for {domain}' if domain else ''}:\n")
    for c in pool: print(format_palette(c, colors))

def cmd_for_project(args, colors, combos):
    path = args[0] if args else "."
    found = set()
    for ext in ['*.css','*.scss','*.tsx','*.jsx','*.ts','*.js','*.html','*.vue','*.svelte','*.json','tailwind.config.*']:
        for fp in glob.glob(os.path.join(path,'**',ext), recursive=True):
            if 'node_modules' in fp or '.git' in fp: continue
            try:
    ...
    if not found: print(f"No custom colors found in {path}"); return
    print(f"\nFound {len(found)} colors. Matching to Wada:\n")
    matched = set()
    for hx in sorted(found)[:12]:
        lab = rgb_to_lab(*hex_to_rgb(hx))
        dists = sorted([(delta_e(lab, c.get("lab", list(rgb_to_lab(*c["rgb_array"])))), c) for c in colors], key=lambda x: x[0])
        best_d, best_c = dists[0]
        if best_d < 35:
            print(f"  {swatch(hx)} {hx} → {best_c['name']} ({best_c['hex']}) ΔE={best_d:.1f}")
            matched.update(best_c["combinations"])
    if matched:
        scores = {}
        for cid in matched: scores[cid] = scores.get(cid,0)+1
        print(f"\nSuggested palettes:")
        for cid, _ in sorted(scores.items(), key=lambda x: -x[1])[:5]:
            if cid in combos: print(format_palette(combos[cid], colors))

def cmd_css(args, colors, combos):
    cid = int(args[0]); prefix = args[args.index("--prefix")+1] if "--prefix" in args else "wada"
    if cid not in combos: print(f"Not found"); return
    combo = combos[cid]; roles = ["primary","secondary","accent","highlight"]
    print(f"/* Sanzo Wada — Combination #{cid} ({combo.get('type','?')}) */")
    if "curated" in combo: print(f"/* {combo['curated']['context']} */")
    print(":root {")
...
def cmd_tokens(args, colors, combos):
    ...
    for i, cc in enumerate(combo["colors"]):
        r = roles[i] if i<len(roles) else f"color-{i+1}"
        tokens["color"][r] = {"value": cc["hex"], "type": "color", "description": cc["name"]}
    tokens["_meta"] = {"source": f"Sanzo Wada #{cid}", "type": combo.get("type","?"), "wcag_aa": combo.get("contrast",{}).get("all_wcag_aa",False)}
...
def cmd_all(args, colors, combos):
    ...
    for cid in sorted(pool.keys()):
        c = pool[cid]; hexes = " ".join(cc["hex"] for cc in c["colors"]); names = ", ".join(cc["name"] for cc in c["colors"])
        w = "✅" if c.get("contrast",{}).get("all_wcag_aa") else "  "; cu = "💡" if "curated" in c else "  "
        print(f"  #{cid:>3}  {hexes:<50} {w}{cu} {names}")

def cmd_stats(args, colors, combos):
    t = len(combos)
    d = len([c for c in combos.values() if c.get("type")=="duo" or 1<=c["id"]<=120])
    tr = len([c for c in combos.values() if c.get("type")=="trio" or 121<=c["id"]<=240])
    q = len([c for c in combos.values() if c.get("type")=="quad" or 241<=c["id"]<=348])
    aa = len([c for c in combos.values() if c.get("contrast",{}).get("all_wcag_aa")])
    al = len([c for c in combos.values() if c.get("contrast",{}).get("all_wcag_aa_large")])
...
COMMANDS = {"palette":cmd_palette,"search":cmd_search,"match":cmd_match,"random":cmd_random,"mood":cmd_mood,"popular":cmd_popular,"accessible":cmd_accessible,"colorblind":cmd_colorblind,"curated":cmd_curated,"for-project":cmd_for_project,"css":cmd_css,"tailwind":cmd_tailwind,"tokens":cmd_tokens,"scss":cmd_scss,"all":cmd_all,"stats":cmd_stats}
